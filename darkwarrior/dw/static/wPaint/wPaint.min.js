...
function(a) {
    ...
    b.prototype = {
        generate: function() {
            function b(b) {
                ...
                d = "canvas" + c,
                e = "ctx" + c;
                return f[d] = document.createElement("canvas"),
                f[e] = f[d].getContext("2d"),
                f["$" + d] = a(f[d]),
                f["$" + d].attr("class", "wPaint-canvas" + (b ? "-" + b: "")).attr("width", f.width + "px").attr("height", f.height + "px").css({
                    position: "absolute",
                    left: 0,
                    top: 0
                }),
            ...
            var f = this;
            b("bg"),
            b("").on("mousedown", c).bindMobileEvents(),
            b("temp").hide(),
            a(document).on("mousemove", d).on("mousedown", a.proxy(this._closeSelectBoxes, this)).on("mouseup", e),
        ...
        _init: function() {
            ...
            b = null;
            this.init = !0;
            for (a in this.options) b = "set" + a.capitalize(),
        ...
        setImage: function(b, c, d, e) {
            function f() {
                ...
                l = h.width,
                m = h.height;
                d || ((h.width > g.width || h.height > g.height || g.options.imageStretch) && (b = g.width / h.width, f = g.height / h.height, a = f > b ? b: f, l = h.width * a, m = h.height * a), j = (g.width - l) / 2, k = (g.height - m) / 2),
        ...
        setCursor: function(b) {
            b = a.fn.wPaint.cursors[b] || a.fn.wPaint.cursors["default"],
            this.$el.css("cursor", 'url("' + this.options.path + b.path + '") ' + b.left + " " + b.top + ", default")
        },
        setMenuOrientation: function(b) {
            a.each(this.menus.all,
            function(a, c) {
                c.options.aligment = b,
                c.setAlignment(b)
            })
        },
        getImage: function(b) {
        ...
        _createModal: function(b) {
            ...
            h.on("click", c),
            f.append(g.append(b)).append(h),
            this.$el.append(e).append(f),
            f.css({
        ...
        _createMenu: function(a, b) {
            return b = b || {},
            b.alignment = this.options.menuOrientation,
            b.handle = this.options.menuHandle,
            new c(this, a, b)
        },
        _fixMenus: function() {
            function b(b, d) {
                ...
                f = e.clone();
                f.appendTo(c.$el),
                f.outerHeight() === f.get(0).scrollHeight && e.css({
                    overflowY: "auto"
                }),
                f.remove()
            }
            var c = this,
            d = null;
            for (var e in this.menus.all) d = c.menus.all[e].$menu.find(".wPaint-menu-select-holder"),
            d.length && d.children().each(b)
        },
        _closeSelectBoxes: function(a) {
            var b, c;
            for (b in this.menus.all) c = this.menus.all[b].$menuHolder.children(".wPaint-menu-icon-select"),
            a && (c = c.not(".wPaint-menu-icon-name-" + a.name)),
            c.children(".wPaint-menu-select-holder").hide()
        },
        ...
        _drawShapeDown: function(a) {
            ...
            this.canvasTempLeftOriginal = a.pageX,
            this.canvasTempTopOriginal = a.pageY
        },
        ...
        _drawDropperDown: function(a) {
            ...
            d = null;
            d = "rgba(" + [c.r, c.g, c.b, c.a].join(",") + ")",
            this.options[this.dropper] = d,
            this.menus.active._getIcon(this.dropper).wColorPicker("color", d)
        },
    ...
    c.prototype = {
        ...
        _createHandle: function() {
            ...
            function c() {
                a.each(e.$menu.data("ui-draggable").snapElements,
                function(a, b) {
                    var c = e.$menu.offset(),
                    d = e.wPaint.menus.primary.$menu.offset();
                    e.dockOffset.left = c.left - d.left,
                    e.dockOffset.top = c.top - d.top,
                    e.docked = b.snapping
                }),
                e._setDrag()
            }
            ...
            "secondary" === this.type && (this.$menu.draggable("option", "snap", this.wPaint.menus.primary.$menu), this.$menu.draggable("option", "start", b), this.$menu.draggable("option", "stop", c), this.$menu.draggable("option", "drag", d)),
            f.bindMobileEvents(),
            f
        },
        _createIconBase: function(b) {
            function c(b) {
                ...
                c.siblings(".hover").removeClass("hover"),
                c.hasClass("disabled") || c.addClass("hover")
            }
            function d(b) {
        ...
        _createIconGroup: function(b) {
            ...
            function e() {
                h.attr("title", b.title).off("click.setIcon").on("click.setIcon", c),
                h.children(".wPaint-menu-icon-img").css(g),
                b.callback.apply(f.wPaint, [])
            }
            var f = this,
            g = {
                backgroundImage: "url(" + b.img + ")"
            ...
            l = null,
            m = 0;
            return i || (h = this._createIconBase(b).addClass("wPaint-menu-icon-group wPaint-menu-icon-group-" + b.group).on("click.setIcon", c).on("mousedown", a.proxy(this._iconClick, this))),
            ...
            k = this._createSelectOption(j, l).addClass("wPaint-menu-icon-name-" + b.name).on("click", e),
            b.after && j.children(".wPaint-menu-select").children(".wPaint-menu-icon-name-" + b.after).after(k),
            i ? void 0 : h
        },
        ...
        _iconClick: function(b) {
            ...
            c.hasClass("disabled") || c.addClass("active")
        },
        _createIconToggle: function(a) {
            function b() {
                d.toggleClass("active"),
                a.callback.apply(c.wPaint, [d.hasClass("active")])
            }
            var c = this,
            d = this._createIconBase(a);
            return d.on("click", b),
        ...
        _createIconSelect: function(b) {
            function c(c) {
                h.children(".wPaint-menu-icon-img").html(a(c.currentTarget).html()),
                b.callback.apply(g.wPaint, [a(c.currentTarget).html()])
            }
            var d, e, f, g = this,
            h = this._createIconBase(b),
            i = this._createSelectBox(h);
            for (d = 0, e = b.range.length; e > d; d++) f = this._createSelectOption(i, b.range[d]),
            f.on("click", c),
            b.useRange && f.css(b.name, b.range[d]);
            return h
        },
        _createSelectBox: function(b) {
            ...
            i = null;
            return g.on("mousedown mouseup", this.wPaint._stopPropagation).on("click", c).hide(),
            g.css("horizontal" === this.options.alignment ? {
                left: 0,
        ...
        _createSelectOption: function(b, c) {
            ...
            return d.children().length || e.addClass("first"),
            d.append(e),
            e
        },
        ...
        _createIconColorPicker: function(a) {
            ...
            function d() {
                f.trigger("click"),
                e.wPaint.dropper = a.name,
                e.wPaint.setMode("dropper")
            }
            var e = this,
        ...
        _createIconMenu: function(a) {
            function b() {
                ...
                b.$menu.toggle(),
                c.handle ? b._setDrag() : b._setPosition()
            }
            var c = this,
    ...
    a.fn.wPaint = function(c, d) {
        ...
        if ("string" == typeof c) {
            ...
            return this.each(g),
            h.length ? 1 === h.length ? h[0] : h: this
        }
        return c = a.extend({},
