...
@login_required
@views_permission
def department(request,*args,**kwargs):  #部门
...
@login_required
@views_permission
def delete_project(request,*args,**kwargs):    #删除项目
    project_archive = kwargs['project']  #项目归档判断
    url = kwargs['project_id']
    user_now = request.session.get('USERNAME',False)
    user_head_portrait = User.objects.get(username=user_now).head_portrait
...
def createtree(request,*args,**kwargs):
    ...
    user_now = request.session.get('USERNAME',False)
    user_head_portrait = User.objects.get(username=user_now).head_portrait
    file_types = File_type.objects.filter(owner_project=0)
    file_names = file_types[0].file_name.all()
    file_root = '/var/centralStorage'
    if request.method == 'POST':
    ...
    if request.method == 'GET':
        ...
        start_file_address = request.GET.get('start_file_address','')
        file_name_start = request.GET.get('file_name_start','')
        file_name_last = request.GET.get('file_name_last')

        if start_file_address == '':
            start_file_address = file_root + '/'
        else:
            start_file_address = file_root + start_file_address
        if file_name_start != '':
            if file_name_last == '':
                file_name = file_name_start.split('_')[0]
            else:
                file_name = file_name_start + file_name_last
        else:
            file_name = ''
        if judge == 'create_file':
            mkdir(start_file_address+file_name)
            return HttpResponseRedirect('/' + url + '/manage/createtree/')
...
@login_required
@views_permission
def file_type(request,*args,**kwargs):  #
    project_archive = kwargs['project']  # 项目归档判断
    url = kwargs['project_id']
    user_now = request.session.get('USERNAME', False)
    user_head_portrait = User.objects.get(username=user_now).head_portrait
    ...
    if request.method == 'GET':
        file_type_id = request.GET.get('type_id')
        if file_type_id is not None and len(file_type_id)>0:
            file_names = []
            for file_name in File_type.objects.get(id=file_type_id).file_name.all():
                response_data = {}
                response_data['type_name'] = file_name.file_name
                response_data['type_id'] = file_name.id
                file_names.append(response_data)
            jstr = dumps(file_names)
...
@login_required
@views_permission
def file_select(request,*args,**kwargs):  #
    project_archive = kwargs['project']  # 项目归档判断
    url = kwargs['project_id']
    user_now = request.session.get('USERNAME', False)
    user_head_portrait = User.objects.get(username=user_now).head_portrait
