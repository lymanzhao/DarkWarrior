...
gantt.copy = function(t) {
    ...
    if (t && "object" == typeof t) {
        for (a = {},
        n = [Array, Date, Number, String, Boolean], e = 0; e < n.length; e++) t instanceof n[e] && (a = e ? new n[e](t) : new n[e]);
...
gantt._on_dblclick = function(t) {
    ...
    if(n!=null){
       window.location.href='/'+$('#project_id').html()+'/task/'+n;
    }
    //end
...
gantt._DnD = function(t, e) {
    ...
    gantt.event(t, "mousedown", gantt.bind(function(n) {
        e.original_target = {
...
gantt._DnD.prototype = {
    dragStart: function(t, e) {
        ...
        i = gantt.bind(function(e) {
            return gantt.eventRemove(document.body, "mousemove", a),
            gantt.eventRemove(document.body, "mouseup", i),
            this.dragEnd(t)
        },
        this);
        gantt.event(document.body, "mousemove", a),
        gantt.event(document.body, "mouseup", i),
    ...
    dragMove: function(t, e) {
        if (!this.config.marker && !this.config.started) {
            ...
            if (s > this.config.sensitivity) {
                if (this.config.started = !0, this.config.ignore = !1, this.callEvent("onBeforeDragStart", [t, this.config.original_target]) === !1) return this.config.ignore = !0,
                !0;
    ...
    dragEnd: function(t) {
        ...
        var links_a = gantt.getLinks(); //甘特图鼠标移动数据传递
        var links_len_a = links_a.length;
        if(links_len!=links_len_a){
            //alert('线段数量有变化'+' 之前'+links_len+' 之后'+links_len_a + ' 新线段ID'+links_a[links_len_a-1].id);
              document.getElementById('source').value=links_a[links_len_a-1].source;
              document.getElementById('target').value=links_a[links_len_a-1].target;
              document.getElementById('type').value=links_a[links_len_a-1].type;
              document.getElementById('add_link').click();


        }
        else{
              //alert(gantt.getTask(task_id).id)
              //alert(gantt.getTask(task_id));//修改任务单
              document.getElementById('id').value= gantt.getTask(task_id).id;
              document.getElementById('start_date').value=gantt.getTask(task_id).start_date;
              document.getElementById('end_date').value=gantt.getTask(task_id).end_date;
              document.getElementById('duration').value=gantt.getTask(task_id).duration;
              document.getElementById('progress').value=gantt.getTask(task_id).progress;
              document.getElementById('text').value=gantt.getTask(task_id).text;
              document.getElementById('change_task').click();
...
gantt._init_grid = function() {
    ...
    this._click.gantt_grid_head_cell = this.bind(function(t, e, n) {
       // alert('add task')
        ...
        if (this.callEvent("onGridHeaderClick", [a, t])) {
            ...
            if (this.config.sort) {
                for (var i, s = a,
                r = 0; this.config.columns.length; r++) if (this.config.columns[r].name == a) {
    ...
    this._click.gantt_add = this.bind(function(t, e, n) {
           //添加子任务
            document.getElementById('parent_id').value = e;
            z = "modal";
            x =".bs-example-modal-lg";
            $('body,html').append('<div data-target="'+x+'" data-toggle="'+z+'" id="add_child_task">'+'</div>');
            $('body,html').delegate('#add_child_task','click',function(){
                
            });
            $('#add_child_task').click();
            $('#add_child_task').remove();
          // setTimeout("$('#add_child_task').remove()",3000)
          //添加子任务
          /*
...
gantt._render_grid_item = function(t) {
    ...
    for (var e = this.getGridColumns(), n = [], a = 0; a < e.length; a++) {
        ...
        o = e[a];
        "add" == o.name ? s = "<div class='gantt_add'></div>": (s = o.template ? o.template(t) : t[o.name], s instanceof Date && (s = this.templates.date_grid(s, t)), s = "<div class='gantt_tree_content'>" + s + "</div>");
        ...
        this.defined(o.align) && (h += "text-align:" + o.align + ";"),

	//添加子任务单model
	//z = "modal";
	//x =".bs-example-modal-lg";
        //i = "<div class='" + _ + "' data-toggle='"+z+"' data-target='"+x+"' style='" + h + "'>" + d + s + "</div>",
        i = "<div class='" + _ + "' style='" + h + "'>" + d + s + "</div>",
        n.push(i)
    }
    ...
    return u.className = "gantt_row" + _,
...
gantt.moveTask = function(t, e, n) {
    ...
    if (t != n) {
        ...
        if (this.callEvent("onBeforeTaskMove", [t, n, e]) !== !1) {
            ...
            this.setParent(i, n),
            this._branches[n] = r,
            i.$level = this.calculateTaskLevel(i),
...
gantt._init_dnd = function() {
    ...
    t.lastTaskOfLevel = function(t) {
        for (var e = gantt._order,
        n = gantt._pull,
        a = null,
        i = 0,
        s = e.length; s > i; i++) n[e[i]].$level == t && (a = n[e[i]]);
        return a ? a.id: null
    },
    t._getGridPos = this.bind(function(t) {
        ...
        a = t.pos.y - 10;
        return a < e.y && (a = e.y),
        a > e.y + this.$grid_data.offsetHeight - this.config.row_height && (a = e.y + this.$grid_data.offsetHeight - this.config.row_height),
        e.x = n,
        e.y = a,
        e
    },
    ...
    t.attachEvent("onDragMove", this.bind(function(e, n) {
        ...
        if (this.isTaskExists(l) || (l = t.lastTaskOfLevel(gantt.config.order_branch_free ? g.$level: 0), l == t.config.id && (l = null)), this.isTaskExists(l)) {
            ...
            for (var u = this.getGlobalTaskIndex(c.id), p = this._pull[this._order[u - 1]], v = 1; (!p || p.id == c.id) && u - v >= 0;) p = this._pull[this._order[u - v]],
            v++;
            if (g.id == c.id) return;
...
gantt._scale_helpers = {
    ...
    adjustSize: function(t, e, n, a) {
        ...
        for (var i = a - n + 1,
        s = this.getSum(e, n, a), r = 0, o = n; a >= o; o++) {
    ...
    setPosSettings: function(t) {
        for (var e = 0,
        n = t.trace_x.length; n > e; e++) t.left.push((t.width[e - 1] || 0) + (t.left[e - 1] || 0));
    },
    ...
    initColSizes: function(t, e, n, a) {
        ...
        e && t.col_width < e && (t.col_width = e, i = t.col_width * s),
        t.width = [];
        for (var r = t.ignore_x || {},
        o = 0; o < t.trace_x.length; o++) r[t.trace_x[o].valueOf()] || t.display_count == t.count ? t.width[o] = 0 : t.width[o] = 1;
    ...
    initScaleConfig: function(t) {
        ...
        t);
        return this.eachColumn(t.unit, t.step,
        function(t) {
            e.count++,
    ...
    iterateScales: function(t, e, n, a, i) {
        ...
        _ = a || r.length - 1,
        d = 0,
        l = 1; l < s.length; l++) for (var g = o; _ >= g; g++) + r[g] != +s[l] || (i && i.apply(this, [d, l, o, g]), o = g, d = l)
    },
    alineScaleColumns: function(t, e, n, a) {
        this.iterateScales(t, e, n, a,
        function(n, a, i, s) {
...
gantt._tasks_dnd = {
    ...
    _resize: function(t, e, n) {
        ...
        n.left ? (t.start_date = gantt.dateFromPos(i.start + e), t.start_date || (t.start_date = new Date(gantt.getState().min_date))) : (t.end_date = gantt.dateFromPos(i.end + e), t.end_date || (t.end_date = new Date(gantt.getState().max_date))),
        ...
        if (2 != t.button) {
            ...
            if (gantt.isTaskExists(n) && (a = gantt.getTask(n)), !gantt._is_readonly(a) && !this.drag.mode) {
                ...
                if (s) if (s.mode && s.mode != gantt.config.drag_mode.ignore && gantt.config["drag_" + s.mode]) {
                    if (n = gantt.locate(e), a = gantt.copy(gantt.getTask(n) || {}), gantt._is_readonly(a)) return this.clear_drag_state(),
                    !1;
    ...
    _fix_dnd_scale_time: function(t, e) {
        ...
        s = gantt._tasks.step;
        gantt.config.round_dnd_dates || (i = "minute", s = gantt.config.time_step),
    ...
    on_mouse_up: function(t) {
        ...
        if (e.mode && e.id) {
            ...
            } else e.obj._dhx_changed = !1,
            gantt.mixin(n, e.obj, !0),
            gantt.updateTask(n.id)
        }
        this.clear_drag_state()

        task_id = n.id //赋值任务单id
    },
...
gantt._path_builder = {
    ...
    get_points: function(t) {
        ...
        o ? (this.point_to(s.down, a), this.point_to(s.right, i)) : (this.point_to(s.right, i), this.point_to(s.down, a)),
        this.point_to(s.right, r);
        else if (t.type == gantt.config.links.finish_to_start) if (o = e.e_x > e.x + 2 * r, this.point_to(s.right, r), o) i -= r,
        ...
        o ? (this.point_to(s.right, i), this.point_to(s.down, a)) : (this.point_to(s.down, a), this.point_to(s.right, i)),
...
gantt._init_links_dnd = function() {
    ...
    function _() {
        return l._direction || (l._direction = document.createElement("div"), gantt.$task_links.appendChild(l._direction)),
        l._direction
    }
    function d() {
    ...
    l.attachEvent("onBeforeDragStart", gantt.bind(function(e, n) {
        ...
        if (gantt._locate_css(a, c)) {
            ...
            r = gantt.getTask(s);
            if (gantt._is_readonly(r)) return i(),
            !1;
            var o = 0;
    ...
    this)),
    l.attachEvent("onAfterDragStart", gantt.bind(function(t, n) {
        e(l.config.marker)
    },
    this)),
    l.attachEvent("onDragMove", gantt.bind(function(a, i) {
        var r = l.config,
        o = l.getPosition(i);
        n(r.marker, o);
        ...
        var b = !(g == _ && d == f && c == p);
        return b && (d && gantt.refreshTask(d, !1), f && gantt.refreshTask(f, !1)),
        b && e(r.marker),
...
gantt._task_renderer = function(t, e, n, a) {
    ...
    this._task_area_renderers[t] ? this._task_area_renderers[t] : (e || this.assert(!1, "Invalid renderer call"), n && n.setAttribute(this.config.layer_attribute, !0), this._task_area_renderers[t] = {
        ...
        render_items: function(t, e) {
            ...
            this.clear(e);
            for (var i = 0,
            s = t.length; s > i; i++) this.render_item(t[i], a);
            e.appendChild(a);
        },
...
gantt._init_tasks = function() {
    function t(t, e, n, a) {
        for (var i = 0; i < t.length; i++) t[i].change_id(e, n),
        t[i].render_item(a)
    }
    this._tasks = {
    ...
    if (t && this.callEvent("onLinkDblClick", [t, e])) {
        ...
        window.setTimeout(function() {
            ...
            function() {
                //删除甘特图链接
                //alert('删除线段');
                document.getElementById('source').value=gantt.getLink(t).source;
                document.getElementById('target').value=gantt.getLink(t).target;
                document.getElementById('type').value=gantt.getLink(t).type;
...
gantt._update_layout_sizes = function() {
    ...
    for (var e = this.$task_data.childNodes,
    n = 0,
    a = e.length; a > n; n++) {
        var i = e[n];
...
gantt._prepare_scale_html = function(t) {
    ...
    n = null,
    a = null,
    i = null; (t.template || t.date) && (a = t.template || this.date.date_to_str(t.date)),
    i = t.css ||
    function() {},
...
gantt._render_tasks_scales = function() {
    ...
    if (this._is_chart_visible()) {
        ...
        for (var r = this._get_resize_options(), o = r.x ? Math.max(this.config.autosize_min_width, 0) : this.$task.offsetWidth, _ = i.prepareConfigs(s, this.config.min_column_width, o, a), d = this._tasks = _[_.length - 1], l = [], g = this.templates.scale_row_class, h = 0; h < _.length; h++) {
            ...
            u = g(_[h]);
            u && (c += " " + u),
            l.push('<div class="' + c + '" style="height:' + _[h].height + "px;line-height:" + _[h].height + 'px">' + this._prepare_scale_html(_[h]) + "</div>")
        }
        t = l.join(""),
...
gantt._render_bg_line = function(t) {
    ...
    if (gantt.config.show_task_cells) for (var i = 0; n > i; i++) {
        ...
        if (s > 0) {
            ...
            d = this.templates.task_cell_class(t, e.trace_x[i]),
            d && (r += " " + d),
            o.className = r,
            a.appendChild(o)
        }
    ...
    a.className = l,
    gantt.config.smart_rendering && (a.style.position = "absolute", a.style.top = this.getTaskTop(t.id) + "px", a.style.width = "100%"),
...
gantt.refreshLink = function(t) {
    if (this._is_render_active()) if (this.isLinkExists(t)) this._render_link(t);
    else for (var e = this._get_link_renderers(), n = 0; n < e.length; n++) e[n].remove_item(t)
},
gantt._combine_item_class = function(t, e, n) {
    ...
    n == i.drag_id && (a.push("gantt_drag_" + i.drag_mode), i.touch_drag && a.push("gantt_touch_" + i.drag_mode));
    ...
    if (r.link_source_id == n && a.push("gantt_link_source"), r.link_target_id == n && a.push("gantt_link_target"), this.config.highlight_critical_path && this.isCriticalTask && this.isCriticalTask(s) && a.push("gantt_critical_task"), r.link_landing_area && r.link_target_id && r.link_source_id && r.link_target_id != r.link_source_id) {
        ...
        g = "";
        g = l ? d ? "link_start_allow": "link_finish_allow": d ? "link_start_deny": "link_finish_deny",
        a.push(g)
    }
    return a.join(" ")
},
gantt._render_pair = function(t, e, n, a) {
    var i = gantt.getState(); + n.end_date <= +i.max_date && t.appendChild(a(e + " task_right")),
...
gantt._task_default_render = function(t) {
    if (!this._isAllowedUnscheduledTask(t)) {
        ...
        s.appendChild(_);
        var d = this._combine_item_class("gantt_task_line", this.templates.task_class(t.start_date, t.end_date, t), t.id); (t.color || t.progressColor || t.textColor) && (d += " gantt_task_inline_color"),
        s.className = d;
        ...
        g && s.appendChild(g),
        this._is_readonly(t) || (n.drag_resize && !this._is_flex_task(t) && o != this.config.types.milestone && gantt._render_pair(s, "gantt_task_drag", t,
        function(t) {
            var e = document.createElement("div");
            return e.className = t,
            e
        }), n.drag_links && this.config.show_links && gantt._render_pair(s, "gantt_link_control", t,
        function(t) {
            var e = document.createElement("div");
            e.className = t,
...
gantt._findBinary = function(t, e) {
    for (var n, a, i, s = 0,
    r = t.length - 1; r >= s;) if (n = Math.floor((s + r) / 2), a = +t[n], i = +t[n - 1], e > a) s = n + 1;
    else {
        if (! (a > e)) return n;
        ...
        return {
            ...
            add: function(t) {
                if (t && this.tempCollection.push(t), this.container()) for (var n = this.container(), a = this.tempCollection, i = 0; i < a.length; i++) {
                    ...
                    r = t.id,
                    o = t.topmost;
                    if (!s.parentNode) if (o) n.appendChild(s);
                    else {
...
gantt.load = function(t, e, n) {
    ...
    this._load_type = a,
    this.ajax.get(t, gantt.bind(function(t) {
        this.on_load(t, a),
...
gantt.attachEvent("onBeforeTaskDisplay",
function(t, e) {
    return ! e.$ignore
}),
...
gantt.xml = {
    _xmlNodeToJSON: function(t, e) {
        for (var n = {},
        a = 0; a < t.attributes.length; a++) n[t.attributes[a].name] = t.attributes[a].value;
        if (!e) {
    ...
    _getCollections: function(t) {
        for (var e = {},
        n = gantt.ajax.xpath("//coll_options", t), a = 0; a < n.length; a++) for (var i = n[a].getAttribute("for"), s = e[i] = [], r = gantt.ajax.xpath(".//item", n[a]), o = 0; o < r.length; o++) {
    ...
    parse: function(t, e) {
        e = this._getXML(t, e);
        for (var n = {},
        a = n.data = [], i = gantt.ajax.xpath("//task", e), s = 0; s < i.length; s++) a[s] = this._xmlNodeToJSON(i[s]);
    ...
    serialize: function() {
        ...
        for (var a = 0,
        i = n.links.length; i > a; a++) e.push(this._copyLink(n.links[a]));
...
gantt._working_time_helper = {
    ...
    _get_unit_order: function(t) {
        for (var e = 0,
        n = this.units.length; n > e; e++) if (this.units[e] == t) return e;
    ...
    is_working_unit: function(t, e, n) {
        ...
        return - 1 == a && (a = this._check_is_working_unit(t, e, n), this._working_units_cache.put(e, t, a)),
        a
    },
    ...
    get_work_units_between: function(t, e, n, a) {
        if (!n) return ! 1;
        for (var i = new Date(t), s = new Date(e), a = a || 1, r = 0; i.valueOf() < s.valueOf();) this.is_working_unit(i, n) && r++,
    ...
    get_closest_worktime: function(t) {
        ...
        for ("past" == t.dir && (d = -1); ! this.is_working_unit(n, e);) {
            ...
            if (n = gantt.date.add(n, d, e), n = gantt._correct_dst_change(n, l, d, e), gantt.date[e + "_start"] && (n = gantt.date[e + "_start"](n)), _ && (s ? a = n: i = n), s = !s, o++, o > r) return gantt.assert(!1, "Invalid working time check"),
            !1
        }
...
gantt.updateTask = function(t, e) {
    ...
    this.callEvent("onBeforeTaskUpdate", [t, e]) === !1 ? !1 : (this._pull[e.id] = e, this._is_parent_sync(e) || this._resync_parent(e), this._isAllowedUnscheduledTask(e) && (this._init_task(e), this._sync_links()), this._update_parents(e.id), this.refreshTask(e.id), this.callEvent("onAfterTaskUpdate", [t, e]), this._sync_order(), void this._adjust_scales())
},
...
gantt.addTask = function(t, e, n) {
    ...
    this.setParent(t, e),
    t = this._init_task(t),
    this.callEvent("onBeforeTaskAdd", [t.id, t]) === !1 ? !1 : (this._pull[t.id] = t, this._add_branch(t, n), this.callEvent("onAfterTaskAdd", [t.id, t]), this.refreshData(), this._adjust_scales(), t.id)
},
...
gantt.createTask = function(t, e, n) {
    if (t = t || {},
    t.id = gantt.uid(), t.start_date || (t.start_date = gantt._default_task_date(t, e)), void 0 === t.text && (t.text = gantt.locale.labels.new_task), void 0 === t.duration && (t.duration = 1), e) {
        this.setParent(t, e);
        var a = this.getTask(e);
        a.$open = !0
    }
...
gantt._init_task_timing = function(t) {
    ...
    void 0 === t.$rendered_type ? t.$rendered_type = e: t.$rendered_type != e && (delete t.$no_end, delete t.$no_start, t.$rendered_type = e),
...
gantt.roundDate = function(t) {
    ...
    s = t.step,
    r = t.unit;
    if (r == gantt._tasks.unit && s == gantt._tasks.step && +i >= +gantt._min_date && +i <= +gantt._max_date) a = Math.floor(gantt._day_index_by_date(i)),
...
gantt.sort = function(t, e, n, a) {
    ...
    if (o) {
        ...
        _.sort(s);
        for (var d = 0; d < _.length; d++) o[d] = _[d].id,
        this.sort(t, e, o[d], !0)
    }
    i && this.render()
},
...
gantt.getParent = function(t) {
    var e = null;
    return e = t.id ? t: gantt.getTask(t),
...
gantt._dp_init = function(t) {
    ...
    t.attachEvent("onAfterUpdateFinish",
    function() { (e || n) && (gantt.batchUpdate(function() {
    ...
    t.attachEvent("onBeforeDataSending",
    function() {
    ...
    t.afterUpdate = function() {
        ...
        var n = t._ganttMode,
        i = e.filePath;
        n = "REST" != this._tMode ? -1 != i.indexOf("gantt_mode=links") ? "links": "tasks": i.indexOf("/link") > i.indexOf("/task") ? "links": "tasks",
        t.setGanttMode(n);
    ...
    t._getRowData = gantt.bind(function(e, n) {
        ...
        return a.$no_start && (a.start_date = "", a.duration = ""),
        a.$no_end && (a.end_date = "", a.duration = ""),
...
gantt._sync_links = function() {
    for (var t = null,
    e = 0,
    n = this._order_full.length; n > e; e++) t = this._pull[this._order_full[e]],
    t.$source = [],
    t.$target = [];
    this._links = [];
...
gantt._get_skin = function(t) {
    ...
    s[1] && "undefined" == typeof s[1].width && (s[1].width = i._second_column_width),
    s[2] && "undefined" == typeof s[2].width && (s[2].width = i._third_column_width),
    i._lightbox_template && (gantt._lightbox_template = i._lightbox_template),
    gantt.resetLightbox()
},
...
gantt.showLightbox = function(t) {
    ...
    if (t<1468000000000){
        if (t && !gantt._is_readonly(this.getTask(t)) && this.callEvent("onBeforeLightbox", [t])) {
            ...
            //var parent_Url = '/' + start[1] + '/newtask/';
            //alert(parent_Url);
            var task_parent_id = gantt.getParent(t); //给前端赋父id值
	    document.getElementById('parent_id').value = task_parent_id;
            //var child_Url = '/' + start[1] + '/newtask/'+task_parent_id;
            //alert(task_parent_id);
            //if (task_parent_id==0){
...
gantt.getLightbox = function(t) {
    if (void 0 === t && (t = this.getLightboxType()), !this._lightbox || this.getLightboxType() != this._get_safe_type(t)) {
        ...
        var n = this._is_lightbox_timepicker(); (gantt.config.wide_form || n) && (e.className += " gantt_cal_light_wide"),
        n && (gantt.config.wide_form = !0, e.className += " gantt_cal_light_full"),
        e.style.visibility = "visible";
        for (var a = this._lightbox_template,
        i = this.config.buttons_left,
        s = 0; s < i.length; s++) {
            var r = this.config._migrate_buttons[i[s]] ? this.config._migrate_buttons[i[s]] : i[s];
            a += "<div class='gantt_btn_set gantt_left_btn_set " + r + "_set'><div dhx_button='1' class='" + r + "'></div><div id='"+ r +"'>" + this.locale.labels[r] + "</div></div>"
        }
        ...
        a += "</div>",
        e.innerHTML = a,
        gantt.config.drag_lightbox && (e.firstChild.onmousedown = gantt._ready_to_dnd, e.firstChild.onselectstart = function() {
            return ! 1
        },
...
gantt._render_sections = function(t) {
    ...
    n = 0; n < t.length; n++) {
        ...
        if (a) {
            ...
            e += "<div id='" + t[n].id + "' class='gantt_cal_lsection'>" + s + this.locale.labels["section_" + t[n].name] + "</div>" + a.render.call(this, t[n]),
            e += "</div>"
        }
    }
    return e
...
gantt._init_lightbox_events = function() {
    ...
    this.event(gantt.getLightbox(), "click",
    function(t) {
        t = t || window.event;
...
gantt._save_lightbox = function() {
    ...
    //var task_list = new Array();//修改任务单
    //task_list[0]=t.id,
    //task_list[1]=t.text,
    //task_list[2]=t.start_date,
    //task_list[3]=t.duration;
    //task_list[4]=t.parent;
    //document.getElementById('task')[0].setAttribute('value', task_list);
    //document.getElementById('task_start_date')[0].setAttribute('value', t.id);
...
gantt.getLightboxValues = function() {
    var t = {};
    gantt.isTaskExists(this._lightbox_id) && (t = this.mixin({},
...
gantt._set_lightbox_values = function(t, e) {
    ...
    for (var i = this._get_typed_lightbox_config(this.getLightboxType()), s = 0; s < i.length; s++) {
        ...
        if (this.form_blocks[r.type]) {
            ...
            _.set_value.call(gantt, o, l, n, r),
            r.focus && _.focus.call(gantt, o)
        }
    }
...
gantt.getLightboxSection = function(t) {
    ...
    r = {
        ...
        node: s,
        getValue: function(t) {
            return gantt.form_blocks[a.type].get_value.call(gantt, s, t || {},
            a)
        },
        setValue: function(t, e) {
            return gantt.form_blocks[a.type].set_value.call(gantt, s, t, e || {},
            a)
        }
    },
...
gantt.form_blocks = {
    getTimePicker: function(t, e) {
        ...
        _ = 0; _ < n.length; _++) {
            ...
            case "%Y":
                ...
                var g, h, c, u;
                t.year_range && (isNaN(t.year_range) ? t.year_range.push && (c = t.year_range[0], u = t.year_range[1]) : g = t.year_range),
                g = g || 10,
                h = h || Math.floor(g / 2),
                c = c || i.getFullYear() - h,
                u = u || c + g;
    ...
    select: {
        ...
        set_value: function(t, e, n, a) {
            var i = t.firstChild; ! i._dhx_onchange && a.onchange && (i.onchange = a.onchange, i._dhx_onchange = !0),
    ...
    time: {
        ...
        set_value: function(t, e, n, a) {
            ...
            this.form_blocks._fill_lightbox_select(s, 0, l, r, i),
            this.form_blocks._fill_lightbox_select(s, r.size, g, r, i)
        },
        get_value: function(t, e, n) {
    ...
    duration: {
        ...
        set_value: function(t, e, n, a) {
            ...
            this),
            l[1].onclick = gantt.bind(function() {
                s(1 * this.config.duration_step)
            },
            this),
            o[0].onchange = i,
            ...
            o[2].onchange = i,
            o[3] && (o[3].onchange = i),
            d.onkeydown = gantt.bind(function(t) {
                t = t || window.event;
            ...
            this),
            d.onchange = gantt.bind(function(t) {
                i()
            },
            this);
    ...
    parent: {
        ...
        _display: function(t, e) {
            ...
            for (var i = t.template || gantt.templates.task_text,
            s = 0; s < n.length; s++) {
...
gantt.dataProcessor.prototype = {
    ...
    setUpdated: function(t, e, n) {
        if (!this._silent_mode) {
            ...
            i && "updated" == n && (n = i),
            e ? (this.set_invalid(t, !1), this.updatedRows[a] = t, this.obj.setUserData(t, this.action_param, n), this._in_progress[t] && (this._in_progress[t] = "wait")) : this.is_invalid(t) || (this.updatedRows.splice(a, 1), this.obj.setUserData(t, this.action_param, "")),
    ...
    _getAllData: function(t) {
        for (var e = {},
        n = !1,
        a = 0; a < this.updatedRows.length; a++) {
    ...
    afterUpdateCallback: function(t, e, n, a) {
        ...
        "wait" != this._in_progress[i] ? (s && this.obj.setUserData(t, this.action_param, ""), delete this._in_progress[i]) : (delete this._in_progress[i], this.setUpdated(e, !0, this.obj.getUserData(t, this.action_param))),
    ...
    afterUpdate: function(t, e, n) {
        if (window.JSON) try {
            ...
            s = a.sid || n[0],
            r = a.tid || n[0];
            return t.afterUpdateCallback(s, r, i, a),
...
gantt._insert_callback = function(t, e, n, a) {
    ...
    "links" == a && (s.add = gantt.addLink, s.isExist = gantt.isLinkExists),
    s.isExist.call(gantt, e) || (i.id = e, s.add.call(gantt, i))
},
gantt._delete_callback = function(t, e, n, a) {
    ...
    "links" == a && (i["delete"] = gantt.deleteLink, i.isExist = gantt.isLinkExists),
...
gantt.init = function(t, e, n) {
    ...
    e && n && (this.config.start_date = this._min_date = new Date(e), this.config.end_date = this._max_date = new Date(n)),
    this._init_skin(),
    this.date.init(),
    ...
    buttons: {
        ...
        "delete": function(t) {
            ...
            function() {
                ...
                document.getElementById('parent').value= e.parent;
...
gantt._get_box_styles = function() {
    ...
    return i.borderBox ? (i.innerWidth -= i.horPaddings, i.innerHeight -= i.vertPaddings) : (i.outerWidth += i.horPaddings, i.outerHeight += i.vertPaddings),
    i
},
...
gantt._set_sizes = function() {
    ...
    if (this._y = t.innerHeight, !(this._y < 20)) {
        ...
        //alert(Math.max(this._y - this.$scroll_hor.offsetHeight - 2, 0))
...
gantt.render = function() {
    if (this._is_render_active()) {
        ...
        if (t && (e = gantt.dateFromPos(t.x + this.config.task_scroll_offset)), this._render_grid(), this._render_tasks_scales(), this._scroll_resize(), this._on_resize(), this._render_data(), this.config.preserve_scroll && t) {
            ...
            a = gantt.dateFromPos(n.x); ( + e != +a || n.y != t.y) && (e && this.showDate(e), gantt.scrollTo(void 0, t.y))
        }
...
gantt._set_scroll_events = function() {
    function t(t) {
        ...
        if (a && Math.abs(a) > Math.abs(i)) {
            ...
            if (gantt.scrollTo(null, d), gantt.$scroll_ver.scrollTop = d, _ == gantt.$scroll_ver.scrollTop) return ! 0
        }
...
gantt._scroll_sizes = function() {
    ...
    return o && (d.x = Math.max(this._x - (_ ? a: 2), 0)),
    _ && (d.y = Math.max(this._y - (o ? a: 0) - this.config.scale_height, 0)),
    d
},
...
gantt.getTaskRowNode = function(t) {
    ...
    a = 0; a < e.length; a++) if (e[a].getAttribute) {
...
gantt.batchUpdate = function(t, e) {
    ...
    this._skip_render = !1,
    e || this.render(),
    a && (this._dp.setUpdateMode(n), this._dp.setGanttMode("tasks"), this._dp.sendData(), this._dp.setGanttMode("links"), this._dp.sendData())
},
gantt.env = {
...
gantt.ajax = {
    ...
    parse: function(t) {
        ...
        window.DOMParser && !gantt.env.isIE ? e = (new window.DOMParser).parseFromString(t, "text/xml") : window.ActiveXObject !== window.undefined && (e = new window.ActiveXObject("Microsoft.XMLDOM"), e.async = "false", e.loadXML(t)),
        e
    },
    ...
    post: function(t, e, n) {
        1 == arguments.length ? e = "": 2 != arguments.length || "function" != typeof e && "function" != typeof window[e] ? e = String(e) : (n = e, e = ""),
    ...
    _call: function(t, e, n, a, i, s, r) {
        ...
        }), "GET" != t || this.cache || (e += (e.indexOf("?") >= 0 ? "&": "?") + "dhxr" + (new Date).getTime() + "=1"), o.open(t, e, a), r) for (var d in r) o.setRequestHeader(d, r[d]);
...
function() {
    ...
    function e(e) {
        if (h) {
            ...
            return gantt.message.keyboard && ((13 == n || 32 == n) && t(h, !0), 27 == n && t(h, !1)),
    ...
    function i(t) {
        c.area || (c.area = document.createElement("DIV"), c.area.className = "gantt_message_area dhtmlx_message_area", c.area.style[c.position] = "5px", document.body.appendChild(c.area)),
        c.hide(t.id);
        var e = document.createElement("DIV");
    ...
    function s(e, n, i) {
        var s = document.createElement("DIV");
        s.className = " gantt_modal_box dhtmlx_modal_box gantt-" + e.type + " dhtmlx-" + e.type,
        ...
        if (r += "</div>", s.innerHTML = r, e.content) {
            ...
            s.childNodes[e.title ? 1 : 0].appendChild(_)
        }
    ...
    function r(t, a, i) {
        ...
        return "top" == t.position ? r.style.top = "-3px": r.style.top = _ + "px",
        r.style.left = o + "px",
        r.onkeydown = e,
        r.focus(),
        t.hidden && gantt.modalbox.hide(r),
        r
    }
    function o(t) {
        return r(t, !0, !1)
...
gantt.date = {
    init: function() {
        for (var t = gantt.locale.date.month_short,
        e = gantt.locale.date.month_short_hash = {},
        n = 0; n < t.length; n++) e[t[n]] = n;
        for (var t = gantt.locale.date.month_full,
    ...
    str_to_date: function(t, e) {
        ...
        a = t.match(/%[a-zA-Z]/g), i = 0; i < a.length; i++) switch (a[i]) {
        case "%j":
        case "%d":
            n += "set[2]=temp[" + i + "]||1;";
            break;
    ...
    parseDate: function(t, e) {
        return "string" == typeof t && (gantt.defined(e) && (e = "string" == typeof e ? gantt.defined(gantt.templates[e]) ? gantt.templates[e] : gantt.date.str_to_date(e) : gantt.templates.xml_date), t = t ? e(t) : null),
        t
    }
},
...
function() {
    ...
    gantt._init_template = function(t, e, n) {
        ...
        n = n || t,
        this.config[t] && a[n] != this.config[t] && (e && this.templates[n] || (this.templates[n] = this.date.date_to_str(this.config[t]), a[n] = this.config[t])),
    ...
    gantt._init_templates = function() {
        ...
        gantt.mixin(this.templates, {
            ...
            drag_link: function(t, e, n, a) {
                ...
                return n && (n = gantt.getTask(n), s += "<b> " + n.text + "</b> " + (a ? i.link_start: i.link_end) + "<br/>"),
                s
            },
...
gantt._init_touch_events = function() {
    ...
    this.config.touch && (window.navigator.msPointerEnabled ? this._touch_events(["MSPointerMove", "MSPointerDown", "MSPointerUp"],
    function(t) {
...
gantt._touch_events = function(t, e, n) {
    ...
    l = null,
    g = null;
    this._gantt_touch_event_ready || (this._gantt_touch_event_ready = 1, gantt.event(gantt.$container, t[0],
    function(t) {
        if (!n(t) && o) {
            l && clearTimeout(l);
            var i = e(t);
            if (gantt._tasks_dnd.drag.id || gantt._tasks_dnd.drag.start_drag) return gantt._tasks_dnd.on_mouse_move(i),
    ...
    gantt.event(this.$container, "contextmenu",
    function(t) {
        return o ? a(t) : void 0
    }),
    gantt.event(this.$container, t[1],
    function(t) {
        if (!n(t)) {
            ...
            l = setTimeout(function() {
                var t = gantt.locate(d); ! t || gantt._locate_css(d, "gantt_link_control") || gantt._locate_css(d, "gantt_grid_data") || (gantt._tasks_dnd.on_mouse_down(d), gantt._tasks_dnd.drag && gantt._tasks_dnd.drag.start_drag && (i(t), gantt._tasks_dnd._start_dnd(d), gantt._touch_drag = !0, gantt.refreshTask(t), gantt._touch_feedback())),
                l = null
            },
            gantt.config.touch_drag)
        }
    }),
    gantt.event(this.$container, t[2],
    function(t) {
        if (!n(t)) {
            l && clearTimeout(l),
            gantt._touch_drag = !1,
            ...
            g && (gantt.refreshTask(gantt.locate(g)), g.parentNode && (g.parentNode.removeChild(g), gantt._touch_feedback())),
