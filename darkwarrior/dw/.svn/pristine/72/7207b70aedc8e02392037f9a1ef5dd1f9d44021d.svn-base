...
@login_required
@views_permission
def effects_view_calendar(request,*args,**kwargs):  #日历
    ...
    return render_to_response('effectstemplates/effects_calendar.html',locals())

@checkCdkey
@login_required
@views_permission
def effects_gantt(request,*aegs,**kwargs):
        ...
        if self=='self':
            ...
            if gantt_row_height:
                ...
                if task_num==0:
                    ...
                    height_num = task_num * 40
            return render_to_response('effectstemplates/effects_gantt.html',locals())
        ...
        if summary is not None and len(summary)>0:
            ...
            if image == 'tiff':
                ...
                create_task = Taskorder.objects.create(summary=summary, creator=creator, description=description,
                                                       ...
                                                       status=statuss[0], owner_project=owner_project,grade = grade,
                ...
                return HttpResponseRedirect('/%s/effects/gantt'%url,locals())
            else:
...
@login_required
@views_permission
def effects_new_child_task(request,*args,**kwargs):    #创建子任务单
    project_archive = kwargs['project']  #项目归档判断
    ...
    return render_to_response('effectstemplates/create_shot.html',locals())


@checkCdkey
@login_required
@views_permission
def effects_newshot(request,*args,**kwargs):    #新建镜头任务单
    project_archive = kwargs['project']  #项目归档判断
...
@login_required
@views_permission
def shot_query(request,*args,**kwargs):
    project_archive = kwargs['project']
    url = kwargs['project_id']
    user_now = request.session.get('USERNAME', None)
    user_head_portrait = User.objects.get(username=user_now).head_portrait
    tasks = Taskorder.objects.filter(owner_project=url,parent=0)
    ...
    task_all =[]
    for task in tasks:
        if Attachment_image.objects.filter(owner=task, owner_project=url).count()>0:
            task_image = Attachment_image.objects.filter(owner=task, owner_project=url).order_by('-time')[0].url
        else:
            if task.task_image:
                task_image = task.task_image
            else:
                task_image = None
        if Taskorder.objects.filter(owner_project=url,parent=task.id).count()>0:

            try:
                key = Taskorder.objects.get(owner_project=url,parent=task.id,milestone=project_type.milestone.filter(name='抠像',owner_project=url)[0])
            ...
            try:
                ...
                erase = Taskorder.objects.get(owner_project=url, parent=task.id,
                                               milestone=project_type.milestone.filter(name='擦除',owner_project=url)[0])
            except:
                erase = None

            try:

                track = Taskorder.objects.get(owner_project=url, parent=task.id,
                                               milestone=project_type.milestone.filter(name='跟踪',owner_project=url)[0])
            except:
                track = None


            try:
                picture = Taskorder.objects.get(owner_project=url, parent=task.id,
                                               milestone=project_type.milestone.filter(name='绘景',owner_project=url)[0])
            except:
                picture = None
            ...
            try:
                ...
                effects = Taskorder.objects.get(owner_project=url, parent=task.id,
                                               milestone=project_type.milestone.filter(name='特效',owner_project=url)[0])
            except:
                effects = None
            ...
            try:
                scene = Taskorder.objects.get(owner_project=url, parent=task.id,
                                               milestone=project_type.milestone.filter(name='场景',owner_project=url)[0])
            except:
                scene = None
            ...
            try:
                ...
                cartoon = Taskorder.objects.get(owner_project=url, parent=task.id,
                                                 milestone=project_type.milestone.filter(name='动画', owner_project=url)[0])
            except:
                cartoon = None
            ...
            try:
                concept = Taskorder.objects.get(owner_project=url, parent=task.id,
                                                   milestone=project_type.milestone.filter(name='概念', owner_project=url)[0])
            except:
                concept = None


            try:
                synthesis = Taskorder.objects.get(owner_project=url, parent=task.id,
                                                   milestone=project_type.milestone.filter(name='合成', owner_project=url)[0])

            except:
                synthesis = None

            task_all.append({'task':task,'key':key,'track':track,'erase':erase,'picture':picture,'effects':effects,'scene':scene,'cartoon':cartoon,
                        'concept':concept,'synthesis':synthesis,'task_image':task_image})

        else:
            task_all.append(
                {'task': task, 'key': None, 'track': None, 'erase': None, 'picture': None, 'effects': None,
                 'scene': None, 'cartoon': None,
                 'concept': None, 'synthesis': None,'task_image':task_image})
    return render_to_response('effectstemplates/shot_query.html',locals())


@checkCdkey
@login_required
@views_permission
def effects_kanban(request,*args,**kwargs):
    project_archive = kwargs['project']  # 项目归档判断
    url = kwargs['project_id']
    staff_user = kwargs['staff_user']
    ...
    sort_list = [{'name': 'milestone', 'name_cn': '里程碑'}, {'name': 'priority', 'name_cn': '优先级'},
                 {'name': 'status', 'name_cn': '状态'},
                 {'name': 'component', 'name_cn': '组件'}, {'name': 'type', 'name_cn': '类型'},
                 {'name': 'versions', 'name_cn': '版本'}, ]
    ...
    if request.method == 'POST':
        ...
        if judge == "copy_task":  # 复制任务
            ...
            if selected_project_id is None:
                return HttpResponseRedirect('/%s/effects/kanban/' % url, locals())
            ...
            return HttpResponseRedirect('/%s/effects/kanban/' % project.id, locals())
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            try:
                ...
                username_error = True
                return render_to_response('effectstemplates/effects_kanban.html', locals())
        except:
            pass
        ...
        if search_content:
            ...
            if present_tasks:
                if len(kanbans) == 0:
                    ...
                    return render_to_response('effectstemplates/effects_load_kanban.html', locals())
...
@login_required
@views_permission
def effects_task_contents(request,*args,**kwargs):
    project_archive = kwargs['project']  # 项目归档判断
    staff_user = kwargs['staff_user']
    url = kwargs['project_id']
    id = kwargs['id']
    ...
    cartoon = None
    concept = None
    if Production_process.objects.filter(owner_task=id, name='抠像').count() > 0:
        key = Production_process.objects.get(owner_task=id, name='抠像')


    if Production_process.objects.filter(owner_task=id, name='擦除').count() > 0:
        erase = Production_process.objects.get(owner_task=id, name='擦除')


    if Production_process.objects.filter(owner_task=id, name='跟踪').count() > 0:
        track = Production_process.objects.get(owner_task=id, name='跟踪')


    if Production_process.objects.filter(owner_task=id, name='绘景').count() > 0:
        picture = Production_process.objects.get(owner_task=id, name='绘景')


    if Production_process.objects.filter(owner_task=id, name='特效').count() > 0:
        effects = Production_process.objects.get(owner_task=id, name='特效')

    if Production_process.objects.filter(owner_task=id, name='场景').count() > 0:
        scene = Production_process.objects.get(owner_task=id, name='场景')

    if Production_process.objects.filter(owner_task=id, name='动画').count() > 0:
        cartoon = Production_process.objects.get(owner_task=id, name='动画')

    if Production_process.objects.filter(owner_task=id, name='概念').count() > 0:
        concept = Production_process.objects.get(owner_task=id, name='概念')

    if Production_process.objects.filter(owner_task=id, name='合成').count() > 0:
        synthesis = Production_process.objects.get(owner_task=id, name='合成')
    ...
    if request.method == 'POST':
        ...
        if judge == 'task_change':
            ...
            if end_date is not None and len(end_date) > 0:
                if end_date != Taskorder.objects.get(id=id).end_date:
                    ...
                    if end_date_timestamp < start_timestamp:
                        ...
                        return HttpResponseRedirect('/%s/effects/task/%s/' % (url, id))
                    else:
                    ...
                    if description != Taskorder.objects.get(id=id).description:
                        ...
                        if Taskorder.objects.get(id=id).owner:
                            if user_now != Taskorder.objects.get(id=id).owner:
                                message_description.append(Taskorder.objects.get(id=id).owner)
                        if Taskorder.objects.get(id=id).creator:
                            if user_now != Taskorder.objects.get(id=id).creator.username:
            ...
            if roll is not None and len(roll) > 0:
                taskorder = Taskorder.objects.get(id=id)
                if roll != taskorder.roll:
                    message_roll = []
                    if taskorder.reporter:
                        if user_now != taskorder.reporter:
                            message_roll.append(taskorder.reporter)
                    if taskorder.owner:
                        for user_name in taskorder.owner.split(','):
                            if user_now != user_name:
                                message_roll.append(user_name)
                    if taskorder.creator:
                        if user_now != taskorder.creator.username:
                            message_roll.append(taskorder.creator.username)
                    message_roll_reminder = list(set(message_roll))
                    for reminder_user in message_roll_reminder:
                        if User.objects.filter(username=reminder_user).exists():
                            message = Message.objects.create(name=roll, classify='task_roll',
                                                         reminder=User.objects.get(username=reminder_user),
                                                         promulgator=user_now, owner_task=taskorder,
                                                         owner_project=Project.objects.get(id=url))
                    Taskorder.objects.filter(id=id).update(roll=roll)
            '''
            if scene is not None and len(scene) > 0:
                taskorder = Taskorder.objects.get(id=id)
                if scene != taskorder.scene:
                    message_scene = []
                    if taskorder.reporter:
                        if user_now != taskorder.reporter:
                            message_scene.append(taskorder.reporter)
                    if taskorder.owner:
                        for user_name in taskorder.owner.split(','):
                            if user_now != user_name:
                                message_scene.append(user_name)
                    if taskorder.creator:
                        if user_now != taskorder.creator.username:
                            message_scene.append(taskorder.creator.username)
                    message_scene_reminder = list(set(message_scene))
                    for reminder_user in message_scene_reminder:
                        if User.objects.filter(username=reminder_user).exists():
                            message = Message.objects.create(name=scene, classify='task_scene',
                                                         reminder=User.objects.get(username=reminder_user),
                                                         promulgator=user_now, owner_task=taskorder,
                                                         owner_project=Project.objects.get(id=url))
                    Taskorder.objects.filter(id=id).update(scene=scene)
            '''

            if grade is not None and len(grade) > 0:
                taskorder = Taskorder.objects.get(id=id)
                if grade != taskorder.grade:
                    '''
                    message_grade = []
                    if taskorder.reporter:
                        if user_now != taskorder.reporter:
                            message_grade.append(taskorder.reporter)
                    if taskorder.owner:
                        for user_name in taskorder.owner.split(','):
                            if user_now != user_name:
                                message_grade.append(user_name)
                    if taskorder.creator:
                        if user_now != taskorder.creator.username:
                            message_grade.append(taskorder.creator.username)
                    message_grade_reminder = list(set(message_grade))
                    for reminder_user in message_grade_reminder:
                        if User.objects.filter(username=reminder_user).exists():
                            message = Message.objects.create(name=grade, classify='task_grade',
                                                         reminder=User.objects.get(username=reminder_user),
                                                         promulgator=user_now, owner_task=taskorder,
                                                         owner_project=Project.objects.get(id=url))
                    '''
                    Taskorder.objects.filter(id=id).update(grade=grade)

            if take is not None and len(take) > 0:
                taskorder = Taskorder.objects.get(id=id)
                if take != taskorder.take:
                    message_take = []
                    if taskorder.reporter:
                        if user_now != taskorder.reporter:
                            message_take.append(taskorder.reporter)
                    if taskorder.owner:
                        for user_name in taskorder.owner.split(','):
                            if user_now != user_name:
                                message_take.append(user_name)
                    if taskorder.creator:
                        if user_now != taskorder.creator.username:
                            message_take.append(taskorder.creator.username)
                    message_take_reminder = list(set(message_take))
                    for reminder_user in message_take_reminder:
                        if User.objects.filter(username=reminder_user).exists():
                            message = Message.objects.create(name=take, classify='task_take',
                                                         reminder=User.objects.get(username=reminder_user),
                                                         promulgator=user_now, owner_task=taskorder,
                                                         owner_project=Project.objects.get(id=url))
                    Taskorder.objects.filter(id=id).update(take=take)
                ...
                if user_now != reply:
                    if User.objects.filter(username=reply).exists():
                        message_2 = Message.objects.create(name='reply', classify='comment',
                                                       reminder=User.objects.get(username=reply), promulgator=user_now,
                                                       owner_task=Taskorder.objects.get(id=id),
                                                       owner_project=Project.objects.get(id=url),
                    ...
                    if comment_filed is not None:  # 评论附件上传
                        ...
                        if len(comment_filed) > 0:
                            if comment_filed_name == 'tiff':
                                ...
                                return render_to_response('effectstemplates/effects_task_content.html', locals())
        ...
        if judge == 'task_update':
            ...
            if status != str(Taskorder.objects.get(id=id).status.id):
                ...
                for reminder_user in message_status_reminder:
                    if User.objects.filter(username=reminder_user).exists():
                        message = Message.objects.create(name=Status.objects.get(id=status).name, classify='task_status',
                                                     reminder=User.objects.get(username=reminder_user),
                                                     promulgator=user_now, owner_task=Taskorder.objects.get(id=id),
                                                     owner_project=Project.objects.get(id=url))
            # 团队
...
def loading_gallery(tasks_list,all_list,loading_num):           #lly 2016/11/1
    for task in tasks_list:
        task_image = task.task_image
        attachment_image = Attachment_image.objects.filter(owner=task, owner_comment=0).order_by('id')
        if task_image or attachment_image.exists():
            all_list.append({'task': task, 'task_image': task_image, 'attachment_image': attachment_image})
        if len(all_list) >= loading_num:
            break
...
@login_required
@views_permission
def gallery(request, *args, **kwargs):  # 故事板      2016/10/31
    project_archive = kwargs['project']  # 项目归档判断
    url = kwargs['project_id']
    staff_user = kwargs['staff_user']
    staff_all_per = kwargs['staff_all_per']
    ...
    sort_list = [{'name': 'milestone', 'name_cn': '里程碑','obj_list':milestones}, {'name': 'priority', 'name_cn': '优先级','obj_list':prioritys},
                 {'name': 'status', 'name_cn': '状态','obj_list':statuss},
                 {'name': 'component', 'name_cn': '组件','obj_list':components}, {'name': 'type', 'name_cn': '类型','obj_list':types},
                 {'name': 'versions', 'name_cn': '版本','obj_list':versions}, ]

    loading_num = 10
    try:
        present_tasks = kwargs['present_tasks']
        page_num = kwargs['page_num']
        present_tasks = list(set(present_tasks.split(',')[0:-1]))
    except:
        present_tasks = False
    ...
    if request.method == 'GET':
        ...
        if search_content and len(search_content.strip())>0:
            ...
            if judge == 'one':
                if user_list:
                    for user_name in user_list:
                        owner_gallery = Taskorder.objects.filter(owner__contains=user_name,owner_project=project_archive).order_by('id')
                        creator_gallery = Taskorder.objects.filter(creator=User.objects.get(username=user_name),owner_project=project_archive).order_by('id')
                        reporter_gallery = Taskorder.objects.filter(reporter=user_name,owner_project=project_archive).order_by('id')
                        task_list_user = list(set(list(owner_gallery) + list(creator_gallery) + list(reporter_gallery)))
                        task_list_user += []
                else:
                    task_list_user = []
                summary = Taskorder.objects.filter(summary__icontains=content,owner_project=project_archive).order_by('id')
                description = Taskorder.objects.filter(description__icontains=content,owner_project=project_archive).order_by('id')
                task_list_content = list(set(list(summary) + list(description)))
                task_list = list(set(task_list_user + task_list_content))
            elif judge == 'two':
                for user_name in user_list:
                    s_owner_gallery = Taskorder.objects.filter(owner__contains=user_name,summary__icontains=content,owner_project=project_archive).order_by('id')
                    s_creator_gallery = Taskorder.objects.filter(creator=User.objects.get(username=user_name), summary__icontains=content,owner_project=project_archive).order_by('id')
                    s_reporter_gallery = Taskorder.objects.filter(reporter=user_name,summary__icontains=content,owner_project=project_archive).order_by('id')
                    d_owner_gallery = Taskorder.objects.filter(owner__contains=user_name,description__icontains=content,owner_project=project_archive).order_by('id')
                    d_creator_gallery = Taskorder.objects.filter(creator=User.objects.get(username=user_name), description__icontains=content,owner_project=project_archive).order_by('id')
                    d_reporter_gallery = Taskorder.objects.filter(reporter=user_name,description__icontains=content,owner_project=project_archive).order_by('id')
                    task_list = list(set(list(s_owner_gallery) + list(s_creator_gallery) + list(s_reporter_gallery) + list(
                                     d_owner_gallery) + list(d_creator_gallery) + list(d_reporter_gallery)))
                    task_list += []
                task_list = list(set(task_list))
            else:
                username_error = True
                return render_to_response('effectstemplates/gallery.html', locals())
            dynamic_loading_kanban(task_list, present_tasks)

            if present_tasks:
                loading_gallery(task_list, all_list, loading_num)
                if len(all_list) < loading_num:
                    return HttpResponse('False')
                else:
                    return render_to_response('effectstemplates/loading_gallery.html', locals())
            else:
                loading_gallery(task_list, all_list, loading_num)
                if len(all_list) == loading_num:
                    more_data = True
            ...
            all_list = []
            all_tasks = list(Taskorder.objects.filter(owner_project=project_archive).order_by('id'))
            dynamic_loading_kanban(all_tasks, present_tasks)
            loading_gallery(all_tasks, all_list, loading_num)
            if present_tasks:
                if len(all_list) < loading_num:
                    return HttpResponse('False')
                else:
